...
export class SceneGenerator {
  ...
  async generateScene(
    ...
    try {
      ...
      const roomDescription =
        await this.imageService.generateRoomDescription(messages);
      steps.push(
        `Generated room description: ${roomDescription.slice(0, 100)}...`,
      );

      onProgress?.(25, "Generating panoramic image...");
      steps.push("Started panoramic image generation");

      const { r2Url: baseImageUrl, imageUrl: tempImageUrl } =
      ...
      if (config.includeIkeaFurniture && this.ikeaService) {
        try {
          ...
          if (furnitureItemsFound === 0) {
            steps.push(
              "No objects detected in the scene, skipping IKEA integration",
            );
            ...
            if (config.enableUpscaling) {
              ...
              try {
                finalImageUrl =
                  await this.imageService.upscaleImage(baseImageUrl);
                steps.push("Image upscaled successfully");
              } catch (error) {
                console.warn(
                  "Image upscaling failed, using original image:",
                  error,
                );
                steps.push(
                  `Image upscaling failed: ${(error as Error).message}`,
                );
              }
            } else {
          ...
          const filteredSegments = await this.ikeaService.filterSegmentsLLM(
            segments,
            baseImageUrl,
          );
          steps.push(
          ...
          const ikeaProducts =
            await this.ikeaService.searchIkeaProducts(filteredSegments);
  ...
  static validateConfig(
    ...
    return {
      includeIkeaFurniture:
        config.includeIkeaFurniture ?? defaults.includeIkeaFurniture,
      enableUpscaling: config.enableUpscaling ?? defaults.enableUpscaling,
      maxRetries: Math.max(
        1,
        Math.min(config.maxRetries ?? defaults.maxRetries, 5),
      ),
      imageWidth: Math.max(
        720,
        Math.min(config.imageWidth ?? defaults.imageWidth, 1440),
      ),
      imageHeight: Math.max(
        360,
        Math.min(config.imageHeight ?? defaults.imageHeight, 720),
      ),
      guidanceScale: Math.max(
        1,
        Math.min(config.guidanceScale ?? defaults.guidanceScale, 20),
      ),
      inferenceSteps: Math.max(
        10,
        Math.min(config.inferenceSteps ?? defaults.inferenceSteps, 50),
      ),
    };
  }
}
