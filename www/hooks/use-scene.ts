...
export function useScene() {
  const {
    data: localSceneData,
    mutate: setLocalSceneData,
    error,
  } = useSWR("scene-data", null, {
  ...
  const setScene = useCallback(
    (updaterFn: (sceneData: SceneData) => SceneData | SceneData) => {
  ...
  const setSceneResult = useCallback(
    (image: string, prompt?: string, title?: string) => {
      setScene((currentScene) => ({
        ...currentScene,
        image,
