...
const CommunityScenesSkeleton = memo(({ isMinified }: { isMinified: boolean }) => (
...
const CommunityScenes = ({ isMinified = false }: CommunityScenesProps) => {
    ...
    const [cache, setCache] = useState<Map<number, (Scene & { latestMessagePart: any[] | null })[]>>(new Map())
    ...
    const loadScenes = useCallback(async (page: number) => {
        if (cache.has(page)) {
            const cachedScenes = cache.get(page)!
            if (page === 0) {
                setScenes(cachedScenes)
            } else {
                setScenes(prev => [...prev, ...cachedScenes])
            }
        ...
        try {
            ...
            setCache(prev => new Map(prev).set(page, newScenes))
    ...
    const validScenes = useMemo(() => {
        return scenes
            .filter(scene =>
                ...
                scene.latestMessagePart[0].data.image &&
                scene.latestMessagePart[0].data.createdAt
            )
            .map(scene => ({
                ...scene,
                base64Image: scene.latestMessagePart![0].data.image,
                createdAt: scene.latestMessagePart![0].data.createdAt
            }))
    }, [scenes])
