            ...
            <SheetContent className="sm:max-w-md">
                <SheetHeader>
                    <SheetTitle className='flex items-center gap-2'>
                        <Home12Icon className='size-4' /> Ikea Items in this Scene</SheetTitle>
                    <SheetDescription>
                ...
                <div className='flex flex-col gap-6 overflow-y-auto p-3'>
                    {ikeaFurniture && ikeaFurniture.length > 0 ? (
                        Array.from(
                            new Map(ikeaFurniture.map(item => [item.payload.product_id, item])).values()
                        ).map((item) => (
                            <div key={item.payload.product_id} className="bg-card border rounded-xl p-6 space-y-4 shadow-sm hover:shadow-md transition-shadow">
                                <div className="flex gap-5">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-3">
                                        <div>
                                            <h3 className="font-semibold text-lg leading-tight">{item.payload.product_name}</h3>
                                            <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{item.payload.description}</p>
                                        </div>
                                ...
                                <div className="flex items-center justify-between pt-2">
                                    <div className="flex items-baseline gap-1">
                                        <span className="font-bold text-xl">${item.payload.price}</span>
                                        <span className="text-sm text-muted-foreground">{item.payload.currency}</span>
                                    </div>
                        ...
                        <div className="flex items-center justify-center h-full py-16">
                            <p className="text-sm text-muted-foreground">No Ikea items found in this scene</p>
                        </div>
                    )}
                </div>
            </SheetContent>
