...
const InteractiveParticleSystem: React.FC<{
  ...
  const { positions, colors, sizes } = useMemo(() => {
    ...
    const categories = [
      ...new Set(points.map((p) => p.payload?.name || "unknown")),
    ];
    ...
    points.forEach((point, i) => {
      ...
      if (i < 5) {
        console.log(
          `Point ${i}: category="${category}", hue=${hue}, color=`,
          color,
        );
      }
        ...
        <div className="space-y-3 pr-16">
          ...
          <div className="space-y-1">
            ...
            <p className="font-medium">
              {point.payload?.product_name || "Unknown"}
            </p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="font-medium">
              {point.payload?.description || "Unknown"}
            </p>
          </div>
          ...
          <div className="space-y-1 flex flex-col">
            ...
            <span className="bg-secondary w-max px-2 py-1 rounded text-xs capitalize">
              {point.payload?.category_name || "unknown"}
            </span>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-primary font-semibold">
              {point.payload?.rating_info.rating || "No rating"}/5 Stars
            </p>
          </div>
        </div>
        <img
          src={point.payload?.main_image_url || ""}
          alt={point.payload?.main_image_alt || ""}
          className="w-full h-full max-w-64 object-cover rounded-lg"
        />
      </div>
    </div>
  );
...
export const VectorVisualization: React.FC<VectorVisualizationProps> = ({
  ...
  const fetchPoints = async () => {
    try {
      ...
      const response = await fetch(
        `/api/qdrant/points?collection=${collectionName}&limit=${maxPoints}&sample=true`,
      );
      if (!response.ok) {
  ...
  return (
    <div className="w-full h-screen bg-background relative">
      ...
      <StatsPanel pointCount={points.length} loading={loading} error={error} />

      <Controls onRefresh={fetchPoints} onResetView={resetView} />
